...
export default function Error500() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <Head>
        <title>{applicationData?.name || 'XinXun'} | 500 - Server Error</title>
        <meta name="description" content={`${applicationData?.name || 'XinXun'} 500 - Server Error`} />
      ...
      <div className="max-w-md w-full">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <button
            onClick={handleRetry}
            disabled={isRetrying}
            className={`flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold transition-all duration-300 shadow-sm ${
              isRetrying
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-200 text-center shadow-sm">
          ...
          <button
            onClick={() => {
              const link_cs = applicationData?.link_cs || localStorage.getItem('link_cs') || 'https://t.me/XinXun_Ai';
